...
export function OverviewScreen({ token, userId }: { token: string; userId: string }) {
  ...
  return <>
    ...
    {!data?.transactionCount ? <div className="border bg-white px-6 py-16 text-center"><Sparkles className="mx-auto size-7 text-primary" /><h2 className="mt-4 text-xl font-semibold">Your clean ledger starts here</h2><p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">Import a CSV or add a transaction manually. Ledgerly will flag duplicates and anything that needs a closer look.</p><Button asChild className="mt-6"><Link href="/import">Import your first statement</Link></Button></div> : <>
      ...
      <section className="mt-4 border bg-white"><div className="border-b px-5 py-4"><h2 className="font-semibold">Top merchants</h2><p className="mt-1 text-xs text-muted-foreground">Merchant totals stay separated by currency.</p></div><div className="grid divide-y xl:grid-cols-2 xl:divide-x xl:divide-y-0">{data.currencySummaries.map((summary) => <div key={summary.currencyCode} className="p-5"><h3 className="mb-3 text-sm font-semibold">{summary.currencyCode}</h3>{summary.merchantTotals.length ? <div className="divide-y">{summary.merchantTotals.slice(0, 6).map((merchant) => <div key={merchant.merchant} className="grid grid-cols-[1fr_auto_auto] gap-4 py-3 text-sm"><span className="truncate font-medium">{merchant.merchant}</span><span className="text-muted-foreground">{merchant.count} entries</span><strong className="text-right text-xs tabular-nums">Spend {money(merchant.spend, summary.currencyCode)}<br />Income {money(merchant.income, summary.currencyCode)}</strong></div>)}</div> : <p className="text-sm text-muted-foreground">No merchant totals yet.</p>}</div>)}</div></section>
